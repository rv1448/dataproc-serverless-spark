...
def copy_and_decompress_data_rapidgzip(gcs_path, hdfs_path):
    """Copy and decompress data using rapidgzip."""
    rgzip_hdfs_path = f"{hdfs_path}_rapidgzip"
    command = f"gsutil cat {gcs_path} | rapidgzip -d | hdfs dfs -put - {rgzip_hdfs_path}"
    start_time = time.time()
    execute_shell_command(command)
    end_time = time.time()
    logging.info(f"{command}")
    logging.info(f"rapidgzip uncompression command execution time: {end_time - start_time:.2f} seconds")
    return rgzip_hdfs_path

def read_data_from_hdfs_write_to_bq(spark, hdfs_path, bq_table):
...
def main():
    ...
    hdfs_path = '/tmp/cluster/part-crimes2-' + id[2]
    
    zcat_hdfs_path = copy_and_decompress_data_zcat(gcs_path, hdfs_path)
    rgzip_hdfs_path = copy_and_decompress_data_rapidgzip(gcs_path, hdfs_path)
    read_data_from_hdfs_write_to_bq(spark, rgzip_hdfs_path, bq_table='demo_dataset.pyspark_batch6')
